    ...
    with tqdm(total=limit) as pbar:
        for line in f:
            ...
            if "1. " in line:
                ...
                moves = line.split(' ')
                white_to_play = True
                record_move(database, white_elo, "white", '*', score)
                record_move(database, black_elo, "black", '*', score)
                move_index = 0
                for move in moves:
                    ...
                    if white_to_play:
                        record_move(database, white_elo, "white", move, score, move_index)
                        white_to_play = False
                    else:
                        record_move(database, black_elo, "black", move, score, move_index)
                        white_to_play = True
    return database
